    ...
    private void Update() {
        ...
        if (player != null && player.selectedObject == GetComponent<GrabbableObject>()) {
            ...
            if (playerCam != null && Physics.Raycast(playerCam.ScreenPointToRay(coor), out hit)) {
                if (hit.collider != null && (hit.collider.gameObject.Equals(gameObject) || hit.collider.transform.parent.gameObject.Equals(gameObject) || (canvas != null && hit.collider.gameObject.Equals(canvas.gameObject)))) {
                    //Debug.LogError("TRUE!");
                    setActive = true;
                }
            }
