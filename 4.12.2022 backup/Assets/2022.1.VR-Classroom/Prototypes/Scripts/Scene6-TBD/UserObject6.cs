...
public class UserObject6 : MonoBehaviour {
    public int ownerID;
    ...
    public void SetOwner(string _id, float[] _f) {
        ...
        if (_f[0] == 1) {
            ...
            if (IsOwner(ASL.GameLiftManager.GetInstance().m_PeerId)) {
                ...
                m_ASLObject.gameObject.GetComponent<PlayerController6>().enabled = true;
                m_ASLObject.gameObject.GetComponent<CharacterController>().enabled = true;
                m_ASLObject.gameObject.GetComponentInChildren<MouseLook6>().enabled = true;
            } else {
